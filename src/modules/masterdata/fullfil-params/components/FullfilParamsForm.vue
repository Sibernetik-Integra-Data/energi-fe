  ...
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @mousedown.self="emit('cancel')">
      <div class="bg-(--surface) rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 flex flex-col gap-5 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between"><h3 class="text-lg font-bold text-(--text) m-0">{{ isEdit ? 'Edit Fullfil Params' : 'Add Fullfil Params' }}</h3><button type="button" class="border-0 bg-transparent text-(--text-soft) text-xl cursor-pointer" @click="emit('cancel')">&times;</button></div>
        <form class="flex flex-col gap-4" @submit.prevent="submit">
          <label class="field">Fullfil Mapping <span class="required">*</span><select v-model="form.fullfil_id" :class="{ invalid: errors.fullfil_id }"><option value="">Select Fullfil Mapping</option><option v-for="item in mappings" :key="item.id" :value="String(item.id)">{{ item.name || `#${item.id}` }}</option></select><span v-if="errors.fullfil_id" class="error">{{ errors.fullfil_id }}</span></label>
          <label class="field">Name <span class="required">*</span><input v-model.trim="form.name" maxlength="50" placeholder="Parameter name" :class="{ invalid: errors.name }" /><span v-if="errors.name" class="error">{{ errors.name }}</span></label>
          ...
          <div class="flex justify-end gap-3"><button type="button" class="button secondary" :disabled="submitting" @click="emit('cancel')">Cancel</button><button type="submit" class="button primary" :disabled="submitting">{{ submitting ? 'Saving…' : (isEdit ? 'Save Changes' : 'Add Fullfil Params') }}</button></div>
        </form>
      </div>
    </div>
  </Teleport>
