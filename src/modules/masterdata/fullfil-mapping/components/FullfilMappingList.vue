<template>
  <div>
    ...
    <template v-else>
      <div class="bg-(--surface) rounded-xl overflow-hidden border border-(--border) shadow-sm max-[1100px]:hidden"><table class="w-full border-collapse table-fixed"><thead><tr class="border-b border-(--border) bg-(--surface-muted)"><th v-for="header in headers" :key="header" class="text-xs font-extrabold text-(--text-muted) py-4 px-3 uppercase tracking-widest text-left">{{ header }}</th></tr></thead><tbody><tr v-if="filteredRows.length === 0"><td colspan="9" class="py-10 text-center text-sm text-(--text-muted)">No data.</td></tr><tr v-for="row in paginatedRows" :key="row.id" class="border-b border-(--border) last:border-b-0 hover:bg-(--surface-muted)"><td class="cell text-center">{{ row.id }}</td><td class="cell font-semibold">{{ row.name || '-' }}</td><td class="cell font-semibold">{{ workName(row.tow_id) }}</td><td class="cell">{{ needName(row.ton_id) }}</td><td class="cell">{{ row.access || '-' }}</td><td class="cell">{{ row.is_view === 0 ? 'No' : 'Yes' }}</td><td class="cell">{{ row.notes || '-' }}</td><td class="cell">{{ formatDate(row.updated_at) }}</td><td class="cell"><div class="flex gap-2"><button type="button" class="action" @click="openEdit(row)">Edit</button><button type="button" class="delete-action" @click="confirmDelete(row)">Delete</button></div></td></tr></tbody></table></div>
      <div class="hidden gap-3 max-[1100px]:flex max-[1100px]:flex-col"><div v-if="filteredRows.length === 0" class="text-center text-sm text-(--text-muted) py-10">No data.</div><div v-for="row in paginatedRows" :key="row.id" class="bg-(--surface) border border-(--border) rounded-xl p-4 shadow-sm"><div class="flex justify-between gap-3"><strong class="text-sm text-(--text)">{{ row.name || `${workName(row.tow_id)} → ${needName(row.ton_id)}` }}</strong><span class="text-xs text-(--text-muted)">ID: {{ row.id }}</span></div><div class="text-xs text-(--text-muted) mt-2">Type of Work: {{ workName(row.tow_id) }} · Type of Need: {{ needName(row.ton_id) }}<br />Access: {{ row.access || '-' }} · Visible: {{ row.is_view === 0 ? 'No' : 'Yes' }}<br />Notes: {{ row.notes || '-' }}</div><div class="flex justify-end gap-2 mt-3"><button type="button" class="action" @click="openEdit(row)">Edit</button><button type="button" class="delete-action" @click="confirmDelete(row)">Delete</button></div></div></div>
      <MasterDataPagination id="fullfil-mapping" :total-items="totalItems" :current-page="currentPage" :page-size="pageSize" :total-pages="totalPages" :visible-pages="visiblePages" @update:current-page="currentPage = $event" @update:page-size="pageSize = $event" />
    </template>
...
const headers = ['ID', 'Name', 'Type of Work', 'Type of Need', 'Access', 'Visible', 'Notes', 'Updated At', 'Actions']
...
const filteredRows = computed(() => { const q = search.value.trim().toLowerCase(); return q ? rows.value.filter((row) => `${row.name || ''} ${workName(row.tow_id)} ${needName(row.ton_id)} ${row.access || ''} ${row.notes || ''}`.toLowerCase().includes(q)) : rows.value })
