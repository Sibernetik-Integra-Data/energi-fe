    ...
    <div
      ...
      <div class="bg-(--surface) rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 flex flex-col gap-5 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-bold text-(--text) m-0">{{ isEdit ? 'Edit Type of Accept' : 'Add Type of Accept' }}</h3>
          <button
            type="button"
        ...
        <form @submit.prevent="onSubmit" class="flex flex-col gap-4" novalidate>
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="toa-name">
          ...
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="toa-detail">Detail</label>
            <textarea
              id="toa-detail"
          ...
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="toa-notes">Notes</label>
            <textarea
              id="toa-notes"
          ...
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="toa-status">Status</label>
            <input
              id="toa-status"
...
const props = defineProps({
  ...
  typeOfAccept: { type: Object, default: null },
  submitting: { type: Boolean, default: false }
})

const emit = defineEmits(['submit', 'cancel'])

const isEdit = computed(() => Boolean(props.typeOfAccept?.id))
...
watch(
  ...
  (val) => {
    if (val) {
      form.value = props.typeOfAccept
        ? {
            name: props.typeOfAccept.name || '',
            detail: props.typeOfAccept.detail || '',
            notes: props.typeOfAccept.notes || '',
            status: props.typeOfAccept.status || ''
          }
        : emptyForm()
      errors.value = {}
    }
