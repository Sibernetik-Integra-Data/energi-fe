<template>
    <div>
        ...
        <div class="flex flex-wrap justify-between items-center gap-4 mb-4">
            <div>
                <h2 class="text-2xl font-extrabold tracking-tight text-(--text) m-0 mb-1">Location List</h2>
                <p class="text-sm text-(--text-muted) m-0">Manage locations</p>
            </div>
            <button
                type="button"
        ...
        <Teleport to="body">
            <div
                ...
                <div class="bg-(--surface) rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 flex flex-col gap-4">
                    <h3 class="text-base font-bold text-(--text) m-0">Delete Location</h3>
                    <p class="text-sm text-(--text-muted) m-0">Delete location <strong>{{ deletingLocation?.name }}</strong>? This action cannot be undone.</p>
...
async function onDeleteConfirm() {
    ...
    try {
        ...
        actionError.value = err?.message || 'Failed to delete location.'
    } finally {
        submitting.value = false
    }
}
