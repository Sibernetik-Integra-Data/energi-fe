<template>
  <div>
    <div class="flex flex-wrap justify-between items-center gap-4 mb-4"><div><h2 class="page-title">Fullfil Payment</h2><p class="text-sm text-(--text-muted) m-0">Manage fullfil payment entries</p></div><button type="button" class="add-button" @click="openCreate">+ Add Fullfil Payment</button></div>
    ...
    <Teleport to="body"><div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40"><div class="bg-(--surface) rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6"><h3 class="font-bold text-(--text)">Delete Fullfil Payment</h3><p class="text-sm text-(--text-muted) my-4">Delete this payment? This action cannot be undone.</p><div v-if="actionError" class="error-text mb-3">{{ actionError }}</div><div class="flex justify-end gap-3"><button class="action" @click="deleteTarget = null">Cancel</button><button class="delete-action" :disabled="submitting" @click="remove">Yes, Delete</button></div></div></div></Teleport>
  </div>
</template>

<script setup>
import { computed, onMounted, ref } from 'vue'
import FullfilPaymentForm from './FullfilPaymentForm.vue'
...
async function save(payload) { submitting.value = true; actionError.value = null; try { if (editing.value?.id) { await updateFullfilPayment(editing.value.id, payload); showToast('Fullfil Payment updated successfully.') } else { const userId = currentUserId(); if (!userId) throw new Error('Unable to determine the logged-in user.'); await createFullfilPayment({ ...payload, user_id: userId }); showToast('Fullfil Payment created successfully.') }; closeForm(); await load() } catch (err) { actionError.value = err?.message || 'Failed to save data.' } finally { submitting.value = false } }
function confirmDelete(row) { deleteTarget.value = row; actionError.value = null }; async function remove() { submitting.value = true; actionError.value = null; try { await deleteFullfilPayment(deleteTarget.value.id); deleteTarget.value = null; showToast('Fullfil Payment deleted successfully.'); await load() } catch (err) { actionError.value = err?.message || 'Failed to delete data.' } finally { submitting.value = false } }; onMounted(load)
</script>
