    ...
    <div
      ...
      <div class="bg-(--surface) rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6 flex flex-col gap-5 max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-bold text-(--text) m-0">{{ isEdit ? 'Edit Type of Need' : 'Add Type of Need' }}</h3>
          <button
            type="button"
        ...
        <form @submit.prevent="onSubmit" class="flex flex-col gap-4" novalidate>
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="ton-name">
          ...
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="ton-detail">Detail</label>
            <textarea
              id="ton-detail"
          ...
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="ton-notes">Notes</label>
            <textarea
              id="ton-notes"
          ...
          <div class="flex flex-col gap-1">
            <label class="text-sm font-semibold text-(--text)" for="ton-status">Status</label>
            <input
              id="ton-status"
...
const props = defineProps({
  ...
  typeOfNeed: { type: Object, default: null },
  submitting: { type: Boolean, default: false }
})

const emit = defineEmits(['submit', 'cancel'])

const isEdit = computed(() => Boolean(props.typeOfNeed?.id))
...
watch(
  ...
  (val) => {
    if (val) {
      form.value = props.typeOfNeed
        ? {
            name: props.typeOfNeed.name || '',
            detail: props.typeOfNeed.detail || '',
            notes: props.typeOfNeed.notes || '',
            status: props.typeOfNeed.status || ''
          }
        : emptyForm()
      errors.value = {}
    }
