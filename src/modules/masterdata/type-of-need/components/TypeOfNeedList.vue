<template>
  <div>
    <div class="flex flex-wrap justify-between items-center gap-4 mb-4">
      <div>
        <h2 class="text-[clamp(24px,2.5vw,36px)] font-semibold leading-tight tracking-[-0.04em] text-(--text) m-0 mb-1">Type of Need</h2>
        <p class="text-sm text-(--text-muted) m-0">Manage type of need entries</p>
      </div>
      <button
        type="button"
    ...
    <div class="flex flex-wrap gap-3 mb-4">
      <input
        ...
        aria-label="Search type of need" />
    </div>
    ...
    <Teleport to="body">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="delete-type-of-need-title"
        ...
        <div class="bg-(--surface) rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6 flex flex-col gap-4">
          <h3 id="delete-type-of-need-title" class="text-base font-bold text-(--text) m-0">Delete Type of Need</h3>
    ...
    <MasterDataPagination id="type-of-need" :total-items="totalItems" :current-page="currentPage" :page-size="pageSize" :total-pages="totalPages" :visible-pages="visiblePages" @update:current-page="currentPage = $event" @update:page-size="pageSize = $event" />
  </div>
</template>

<script setup>
import { computed, onMounted, ref } from 'vue'
import TypeOfNeedForm from './TypeOfNeedForm.vue'
import { createTypeOfNeed, deleteTypeOfNeed, listTypeOfNeed, updateTypeOfNeed } from '../model'
...
async function loadData() {
  ...
  try {
    rows.value = await listTypeOfNeed()
    await resolveAuditUsers(rows.value)
  } catch (err) {
    console.error('[TypeOfNeed] Failed to load:', err)
...
function openEdit(typeOfNeed) {
  editingType.value = { ...typeOfNeed }
  actionError.value = null
  showForm.value = true
}
...
async function onFormSubmit(formData) {
  ...
  try {
    if (editingType.value?.id) {
      await updateTypeOfNeed(editingType.value.id, formData)
      showToast('Type of Need updated successfully.')
    } else {
      await createTypeOfNeed(formData)
      showToast('Type of Need created successfully.')
    }
    closeForm()
    await loadData()
  } catch (err) {
    console.error('[TypeOfNeed] Save failed:', err)
...
function confirmDelete(typeOfNeed) {
  deletingType.value = typeOfNeed
  actionError.value = null
...
async function onDeleteConfirm() {
  ...
  actionError.value = null
  try {
    await deleteTypeOfNeed(deletingType.value.id)
    showDeleteConfirm.value = false
    deletingType.value = null
    showToast('Type of Need deleted successfully.')
    await loadData()
  } catch (err) {
    console.error('[TypeOfNeed] Delete failed:', err)
