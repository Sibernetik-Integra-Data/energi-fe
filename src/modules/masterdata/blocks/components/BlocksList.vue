<template>
    <div>
        ...
        <div class="flex flex-wrap justify-between items-center gap-4 mb-4">
            <div>
                <h2 class="text-[clamp(24px,2.5vw,36px)] font-semibold leading-tight tracking-[-0.04em] text-(--text) m-0 mb-1">Blocks List</h2>
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="border-b border-(--border) bg-(--surface-muted)">
                            ...
                            <th class="text-left text-xs font-extrabold text-(--text-muted) py-4 px-6 uppercase tracking-widest">Information</th>
                    ...
                    <tbody>
                        ...
                        <tr
                            ...
                            <td class="py-4 px-6 align-middle text-sm text-(--text-muted)">{{ row.information || '-' }}</td>
            ...
            <div class="hidden gap-3 max-[920px]:flex max-[920px]:flex-col">
                ...
                <div
                    ...
                    <div class="text-xs text-(--text-muted)">
                        <span class="font-semibold text-(--text)">Title:</span> {{ row.title || '-' }}
                    </div>
                    <div class="text-xs text-(--text-muted)">
                        <span class="font-semibold text-(--text)">Information:</span> {{ row.information || '-' }}
                    </div>
        ...
        <MasterDataPagination id="blocks" :total-items="totalItems" :current-page="currentPage" :page-size="pageSize" :total-pages="totalPages" :visible-pages="visiblePages" @update:current-page="currentPage = $event" @update:page-size="pageSize = $event" />
</template>
