    ...
    <div class="bg-(--surface) border border-(--border) rounded-2xl overflow-hidden shadow-sm flex flex-col py-2 gap-2">
        ...
        <div class="p-5 pb-3 flex justify-between items-start gap-3">
            ...
            <div class="w-11 h-11 rounded-xl flex items-center justify-center shrink-0">
                <img v-if="iconSrc" :src="iconSrc" class="w-6 h-6" alt="" />
            </div>
        </div>
        ...
        <div class="px-5 pb-3">
            ...
            <div class="flex flex-wrap gap-1.5">
                ...
                <button
                    ...
                    {{ showAllBlocks ? "Tutup" : "+" + hiddenCount + " more" }}
                </button>
            </div>
        </div>
...
// Icons for sensus type of work card section
...
const visibleBlocks = computed(() => (showAllBlocks.value ? props.blocks : props.blocks.slice(0, props.maxVisible)));
const hiddenCount = computed(() => (showAllBlocks.value ? 0 : Math.max(0, props.blocks.length - props.maxVisible)));

const iconMap = {
    pembersihan: cleaningIcon,
    pemupukan: fertilizeIcon,
    panen: harvestIcon,
};
...
const iconSrc = computed(() => {
    ...
    for (const [keyword, src] of Object.entries(iconMap)) {
        if (key.includes(keyword)) return src;
    }
    return null;
});
