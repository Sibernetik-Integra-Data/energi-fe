...
function isPembersihan(item) {
  ...
  return (
    groupOfWorkName.includes('pembersih') ||
    typeOfWorkName.includes('pembersih') ||
    typeOfWorkName.includes('bersih') ||
    typeOfWorkName.includes('clean')
  )
}
...
function mapPlanItem(p) {
  ...
  return {
    ...
    jobType: p.sensus_detail?.type_of_work?.name || 'Pembersihan',
...
export function createPembersihanModel() {
  return {
    getNavigation() {
    ...
    getIntro() {
      return {
        title: 'Penugasan Pembersihan',
        description: 'Daftar penugasan pembersihan kebun berdasarkan data perencanaan.'
      }
    }
  }
}
...
export async function loadPembersihanList(filters = {}) {
  ...
  // Filter to only pembersihan items (client-side)
  const filtered = allItems.filter((item) => {
    const rawRow = rows.find((r) => r.id === item.id)
    return isPembersihan(rawRow)
  })
...
export async function loadPembersihanDetail(planId) {
