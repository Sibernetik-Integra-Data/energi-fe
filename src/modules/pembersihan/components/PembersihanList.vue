<template>
  <div>
    ...
    <div class="mb-5 flex flex-col gap-3">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-2 bg-(--surface-muted) border border-(--border) p-2 rounded-xl items-center">
        <input
          v-model="searchQuery"
          type="text"
    ...
    >
      ...
      >
        <div class="p-5 pb-3 flex justify-between items-start gap-3">
          <div class="min-w-0 flex flex-col gap-1">
            <div class="flex items-center gap-2">
              <span class="w-9 h-9 rounded-xl bg-teal-100 flex items-center justify-center text-lg shrink-0" aria-hidden="true">🧹</span>
              <div class="min-w-0">
                <h3 class="text-sm font-bold text-(--text) leading-snug truncate">{{ item.jobType }}</h3>
                <p class="text-xs text-(--text-muted) truncate">Sensus {{ item.sensusId || '—' }}</p>
              </div>
            </div>
          </div>
          <span :class="statusChipClass(item.status)" class="shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full">
        ...
        <div v-if="item.blocks.length > 0" class="px-5 pb-3">
          <p class="text-[10px] font-semibold text-(--text-muted) mb-1.5 tracking-wide uppercase">Nomor Petak</p>
  ...
  if (searchQuery.value.trim()) {
    ...
    result = result.filter(
      (item) =>
        ...
        String(item.sensusId || '').toLowerCase().includes(q) ||
