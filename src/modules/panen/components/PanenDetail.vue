        ...
        <template v-else-if="plan">
            <div class="bg-(--surface) rounded-xl p-6 flex flex-col gap-6">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <img :src="taskIcon" alt="Ikon panen" class="w-14 h-14 shrink-0" />
                </div>

                <div class="flex items-start gap-2 font-inter" v-if="plan.sensusDateFormatted || plan.sensusDate">
                    <span class="w-25 shrink-0 text-xs leading-5 text-(--text-muted)"
                        >Tanggal Sensus</span
                    >
                ...
                <ProtectedImage v-if="plan.photo" :src="plan.photo" alt="Foto panen" container-class="mt-4 rounded-xl overflow-hidden bg-(--surface-muted)" image-class="w-full max-h-72 object-cover" />
